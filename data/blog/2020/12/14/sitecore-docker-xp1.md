...
前回紹介をした記事 [Sitecore 10 を Docker で実行する](/blog/2020/12/11/sitecore10docker) では、XP0 の環境を立ち上げました（ CM と CD が同じ）。ドメイン名としては localhost というドメインで設定を進めたため、証明書は独自証明書を作成、利用して、テスト環境を立ち上げた形になります。
