...
また、今回は Azure App Service for Seller Admin に関しても Vercel に展開する手順を紹介しますが、その際は Azure App Service for Seller Admin のインスタンス作成も省略します。
...
を true に変更します。続いてリソースの設定をするために、 Seller\src\assets\appConfigs\defaultadmin-test.json のファイルを開き、Azure のストレージの URL を変更してください。また、OrderCloud の管理画面から、Back Office App の clientID を取得して、設定をします。
