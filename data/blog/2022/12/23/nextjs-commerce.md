...
```
npx @ordercloud/seeding seed https://raw.githubusercontent.com/ordercloud-api/ordercloud-seed/main/seeds/Vercel-B2C.yml -n='Nextjs Commerce Japan' -u={username} -p={password} -r=jpn
```
...
この画面画面からは、API Client を選択して値を取得していきます。まずは、 Vercel-Middleware-Connector から以下の 2 つのキーを取得してください。
...
最後に Buyer の ID を取得するために、 Vercel-Storefront-Connector に切り替えて ID を取得します。
